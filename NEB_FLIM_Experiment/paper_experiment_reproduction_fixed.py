...
class PaperExperimentReproductionFixed:
    ...
    def experiment_1_prior_estimation(self, n_values: List[int], L_values: List[int], 
        ...
        results = {}
        # Store prior distributions for plotting
        results['prior_distributions'] = {}
        
        for n in n_values:
            results[n] = {}
            # Store prior distribution for specific photon numbers (100, 1000)
            if n in [100, 1000]:
            ...
            for L in L_values:
                ...
                for repeat in range(num_repeats):
                    ...
                    try:
                        ...
                        # Store true prior distribution for plotting (only for 800 intervals)
                        if n in [100, 1000] and L == 800 and repeat == 0:
                            # Create histogram of true lifetimes
                            hist, bin_edges = np.histogram(true_taus, bins=50, range=(0.5, 3.5), density=True)
    ...
    def plot_results(self, results: Dict, save_path: str = "paper_experiment_results_fixed.png"):
        ...
        # Experiment 1: Prior distribution curves for photons 100 and 1000
        if 'experiment_1' in results and 'prior_distributions' in results['experiment_1']:
            ax = axes[0]
            colors = ['blue', 'red']
            photon_numbers = [100, 1000]
            
            for i, n_photons in enumerate(photon_numbers):
                if n_photons in results['experiment_1']['prior_distributions']:
                    prior_data = results['experiment_1']['prior_distributions'][n_photons]
                    if prior_data:  # Check if data exists
                        ax.plot(prior_data['tau_values'], prior_data['probabilities'], 
                               'o-', color=colors[i], label=f'{n_photons} photons')
